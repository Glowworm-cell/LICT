...
GPT_generate_gene_text <- function(positive_gene, negative_gene) {
  ...
  final_instruction <- "Based on the additional information above, modify my previous response and list all cell types, including those that have not been modified."

  # Combine all non-empty lines
  if (length(result_texts) > 0) {
    full_text <- paste(result_texts, collapse = "\n")
    full_text <- paste(full_text, final_instruction, sep = "\n")
  } else {
