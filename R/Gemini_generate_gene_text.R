...
Gemini_generate_gene_text <- function(positive_gene, negative_gene) {
  ...
  first_instruction <- "Based on the additional information below, modify my previous answer and list all cell types, including those that have not been modified."

  # Combine all non-empty lines
  if (length(result_texts) > 0) {
    full_text <- paste(result_texts, collapse = "\n")
    full_text <- paste(first_instruction, full_text,  sep = "\n")
  } else {
    full_text <- first_instruction
  }
}
